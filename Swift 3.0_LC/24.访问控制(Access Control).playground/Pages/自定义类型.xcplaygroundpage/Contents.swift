...
public class SomePublicClass {                  // 显式的 public 类
    ...
    fileprivate func someFilePrivateMethod() {} // 显式的 fileprivate 类成员
...
fileprivate class SomeFilePrivateClass {        // 显式的 fileprivate 类
    func someFilePrivateMethod() {}             // 隐式的 fileprivate 类成员
    private func somePrivateMethod() {}         // 显式的 private 类成员
}
...
//: 以下该函数返回类型的访问级别是 `private，`所以你必须使用 `private` 修饰符，明确指定该函数的访问级别：
/*
