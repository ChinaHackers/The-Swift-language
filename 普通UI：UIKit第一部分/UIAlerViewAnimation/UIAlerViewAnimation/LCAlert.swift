...
let MAIN_COLOR = UIColor(red: 152/255.0, green: 97/255.0, blue:
    170/255.0, alpha: 1.0)

class LCAlert: UIViewController {
    ...
    private func resizeAndRelayout() {
        ...
        //进入时的动画
        //逆时针旋转
//         contentView.transform = CGAffineTransformMakeRotation(-SCREEN_HEIGHT/4)
        
        //顺时针旋转
        contentView.transform = CGAffineTransformMakeRotation(-SCREEN_WIDTH/8)
...
//        contentView.transform = CGAffineTransformMakeTranslation(0, -SCREEN_HEIGHT/2)
        ...
        //缩放
//        contentView.transform = CGAffineTransformMakeScale(0, -SCREEN_WIDTH/2)
        
        
        UIView.animateWithDuration(1.0, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: UIViewAnimationOptions.CurveEaseIn, animations: { () -> Void in
...
extension LCAlert{
    ...
    func showAlert(title: String, subTitle: String?,buttonTitle: String ,otherButtonTitle:String?,action:((OtherButton: UIButton) -> Void)) {
        ...
        if buttonTitle.isEmpty == false {
            ...
            button.addTarget(self, action: #selector(LCAlert.doCancel(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            button.tag = 0
        ...
        if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
            ...
            button.backgroundColor = MAIN_COLOR
            button.addTarget(self, action: #selector(LCAlert.pressed(_:)), forControlEvents: UIControlEvents.TouchUpInside)
