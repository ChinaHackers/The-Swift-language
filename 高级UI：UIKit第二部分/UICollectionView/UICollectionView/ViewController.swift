...
class ViewController: UICollectionViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = self.collectionView!.dequeueReusableCell(withReuseIdentifier: identify, for: indexPath)
        
        // 从界面查找到控件元素并设置属性
        (cell.contentView.viewWithTag(1) as! UIImageView).image = UIImage(named: courses[(indexPath as NSIndexPath).item]["pic"]!)
        
        (cell.contentView.viewWithTag(2) as! UILabel).text = courses[(indexPath as NSIndexPath).item]["name"]
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        print("选择了\((indexPath as NSIndexPath).item)")
    }
